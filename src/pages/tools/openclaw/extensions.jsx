...
import { 
  ...
  Target, Zap, Puzzle, Link as LinkIcon, ShieldCheck, Box, Terminal, Wifi
} from 'lucide-react';
...
const NeuralNetwork = () => {
    ...
    useFrame((state) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            pointsRef.current.geometry.attributes.position.array[i * 3 + 1] = y + Math.cos(time + x * 0.9) * 0.035;
        }
...
export default function ExtensionsPage() {
    const content = pageContent['extensions'];
    ...
    return (
        <div className="min-h-screen bg-background transition-colors duration-500 overflow-hidden relative text-foreground font-body">
            ...
            <main className="relative z-10 pt-32 pb-24">
                ...
                <section className="px-8 lg:px-20 py-20 max-w-7xl mx-auto">
                    <div className="max-w-4xl">
                        ...
                        >
                            ...
                            <span className="text-xs font-mono text-accent uppercase tracking-[0.3em] font-bold">Execution · Integration · Scale</span>
                        </motion.div>
                        ...
                        >
                            <Link to="/contact" className="px-10 py-5 bg-accent text-white font-bold rounded-2xl flex items-center gap-3 hover:scale-105 transition-all shadow-xl shadow-accent/20 text-lg uppercase tracking-widest font-display">
                                Get Extensions <ArrowUpRight size={20} />
                            </Link>
                        </motion.div>
                    </div>
                </section>
                ...
                <section className="px-8 lg:px-20 py-24 border-y border-border/50 bg-card-bg/5 backdrop-blur-xl">
                    <div className="max-w-7xl mx-auto grid md:grid-cols-[1fr_450px] gap-20 items-center">
                        <div className="glass-card p-12 md:p-16 border-l-4 border-l-accent relative overflow-hidden group">
                            ...
                            <h2 className="text-3xl md:text-4xl font-display font-bold mb-8 italic">"Bridge the <span className="text-accent underline decoration-accent/20 underline-offset-8">Output Gap</span>."</h2>
                            <div className="space-y-6 text-secondary text-lg leading-relaxed font-body">
                                <p>{content.whatItIs}</p>
                                <p>
                                    Extensions provide the vital link between high-level reasoning and low-level execution. By exposing secure API endpoints to your AI agents, we enable them to orchestrate complex tasks across your entire software stack with autonomous precision.
                                </p>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { icon: <LinkIcon />, title: "Connected", desc: "Seamless API handshakes." },
                                { icon: <ShieldCheck />, title: "Sanitised", desc: "Strict data privacy layers." },
                                { icon: <Box />, title: "Modular", desc: "Plug-and-play capability." },
                                { icon: <Wifi />, title: "Live", desc: "Real-time web interaction." }
                            ].map((item, i) => (
                                <motion.div 
                                    key={i}
                ...
                {/* 3. The Integration Flow (Process) */}
                <section className="px-8 lg:px-20 py-32 max-w-7xl mx-auto">
                    <div className="text-center mb-24">
                        <h2 className="section-title mb-6 font-display uppercase tracking-widest">Deployment <span className="text-accent underline decoration-accent/10">Cycle</span></h2>
                        <p className="text-secondary max-w-xl mx-auto text-lg font-body">Standardized protocols for extending AI operational reach.</p>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16 text-center relative z-10">
                        {content.results.map((metric, i) => (
                            <div key={i} className="flex flex-col items-center p-8 border-r last:border-0 border-white/10 group">
                                <Terminal className="w-12 h-12 mb-8 opacity-40 group-hover:opacity-100 transition-opacity" />
                ...
                <section className="px-8 lg:px-20 py-32 border-b border-border/50">
                    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-24">
                        <div className="space-y-16">
                            <div>
                                <h2 className="section-title !text-4xl text-left italic mb-8">Structural <span className="text-accent underline decoration-accent/20">Advantages</span></h2>
                                <p className="text-secondary text-lg mb-12 font-body max-w-xl">
                                    Our extensions are built on a high-availability serverless architecture, ensuring zero latency between AI command and physical execution.
                                </p>
                            </div>
                        ...
                        <div className="glass-card !p-12 space-y-12 bg-card-bg/20 border-l border-border/50">
                            ...
                            <div className="flex flex-col gap-10">
                                <h3 className="text-xs font-bold uppercase tracking-[0.5em] text-accent flex items-center gap-4">
                                    <Zap size={20} /> Execution Blockers
                                </h3>
                ...
                <section className="px-8 lg:px-20 py-24 text-center">
                    <div className="glass-card max-w-4xl mx-auto p-16 md:p-24 border-t-8 border-t-accent shadow-2xl relative overflow-hidden group">
                        ...
                        <h2 className="text-5xl md:text-7xl font-display font-bold mb-10 tracking-tighter">Extend the <span className="text-accent italic">Limit</span></h2>
                        <p className="text-xl text-secondary mb-12 max-w-xl mx-auto leading-relaxed font-body">
                            Ready to give your AI agents the tools they need to actually do the work? Let's build your plugin stack.
                        </p>
                        <Link to="/contact" className="px-12 py-6 bg-accent text-white font-bold rounded-2xl text-xl hover:scale-105 transition-transform inline-flex items-center gap-4 shadow-2xl shadow-accent/20 font-display uppercase tracking-widest">
                            Install Plugins <ArrowUpRight size={24} />
                        </Link>
                    </div>
                </section>
            </main>
