...
import { 
  ...
  Target, Zap, Cpu, Settings, Layers, Code, ZapOff, Activity
} from 'lucide-react';
...
const NeuralNetwork = () => {
    ...
    useFrame((state) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            pointsRef.current.geometry.attributes.position.array[i * 3 + 1] = y + Math.cos(time + x * 0.8) * 0.04;
        }
...
export default function AutomationSkillsPage() {
    const content = pageContent['automation'];
    ...
    return (
        <div className="min-h-screen bg-background transition-colors duration-500 overflow-hidden relative text-foreground font-body">
            ...
            <main className="relative z-10 pt-32 pb-24">
                ...
                <section className="px-8 lg:px-20 py-20 max-w-7xl mx-auto">
                    <div className="max-w-4xl">
                        ...
                        >
                            ...
                            <span className="text-xs font-mono text-accent uppercase tracking-[0.3em] font-bold">Workflow Intelligence · Library</span>
                        </motion.div>
                        ...
                        >
                            <Link to="/contact" className="px-10 py-5 bg-accent text-white font-bold rounded-2xl flex items-center gap-3 hover:scale-105 transition-all shadow-xl shadow-accent/20 text-lg uppercase tracking-widest font-display">
                                Access Library <ArrowUpRight size={20} />
                            </Link>
                        </motion.div>
                    </div>
                </section>
                ...
                <section className="px-8 lg:px-20 py-24 border-y border-border/50 bg-card-bg/5 backdrop-blur-xl">
                    <div className="max-w-7xl mx-auto grid md:grid-cols-[1fr_450px] gap-20 items-center">
                        <div className="glass-card p-12 md:p-16 border-l-4 border-l-accent relative overflow-hidden group">
                            ...
                            <h2 className="text-3xl md:text-4xl font-display font-bold mb-8 italic">"Pre-Configured <span className="text-accent underline decoration-accent/20 underline-offset-8">Reliability</span>."</h2>
                            <div className="space-y-6 text-secondary text-lg leading-relaxed font-body">
                                <p>{content.whatItIs}</p>
                                <p>
                                    Our Automation Skills Library provides the structural blueprints for high-velocity business growth. These aren't just templates; they are battle-tested logical sequences that have already processed millions of data points across our client base.
                                </p>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { icon: <Settings />, title: "n8n Core", desc: "Low-code node power." },
                                { icon: <Zap />, title: "Make Scenarios", desc: "Visual logic flow." },
                                { icon: <Code />, title: "Custom JS", desc: "Complex data manipulation." },
                                { icon: <Layers />, title: "Atomic", desc: "Independently deployable." }
                            ].map((item, i) => (
                                <motion.div 
                                    key={i}
                ...
                {/* 3. The Implementation Stack (Process) */}
                <section className="px-8 lg:px-20 py-32 max-w-7xl mx-auto">
                    <div className="text-center mb-24">
                        <h2 className="section-title mb-6 font-display uppercase tracking-widest">Rapid <span className="text-accent underline decoration-accent/10">Orchestration</span></h2>
                        <p className="text-secondary max-w-xl mx-auto text-lg font-body">Standardized deployment for high-performance automation teams.</p>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16 text-center relative z-10">
                        {content.results.map((metric, i) => (
                            <div key={i} className="flex flex-col items-center p-8 border-r last:border-0 border-white/10 group">
                                <Activity className="w-12 h-12 mb-8 opacity-40 group-hover:opacity-100 transition-opacity" />
                ...
                {/* 5. Toolchain Capabilities */}
                <section className="px-8 lg:px-20 py-32 border-b border-border/50">
                    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-24">
                        <div className="space-y-16">
                            <div>
                                <h2 className="section-title !text-4xl text-left italic mb-8">Infrastructure <span className="text-accent underline decoration-accent/20">Protocols</span></h2>
                                <p className="text-secondary text-lg mb-12 font-body max-w-xl">
                                    Each skill is version-controlled and includes self-healing logic to ensure maximum uptime for your critical business operations.
                                </p>
                            </div>
                        ...
                        <div className="glass-card !p-12 space-y-12 bg-card-bg/20 border-l border-border/50">
                            <div className="flex flex-col gap-10">
                                <h3 className="text-xs font-bold uppercase tracking-[0.5em] text-accent flex items-center gap-4">
                                    <Target size={20} /> Deployment Target
                                </h3>
                            ...
                            <div className="flex flex-col gap-10">
                                <h3 className="text-xs font-bold uppercase tracking-[0.5em] text-accent flex items-center gap-4">
                                    <ZapOff size={20} /> Friction Points
                                </h3>
                ...
                <section className="px-8 lg:px-20 py-24 text-center">
                    <div className="glass-card max-w-4xl mx-auto p-16 md:p-24 border-t-8 border-t-accent shadow-2xl relative overflow-hidden group">
                        ...
                        <h2 className="text-5xl md:text-7xl font-display font-bold mb-10 tracking-tighter">Accelerate Your <span className="text-accent italic">Build</span></h2>
                        <p className="text-xl text-secondary mb-12 max-w-xl mx-auto leading-relaxed font-body">
                            Stop building from scratch. Access the repository that high-performance automation teams use to scale.
                        </p>
                        <Link to="/contact" className="px-12 py-6 bg-accent text-white font-bold rounded-2xl text-xl hover:scale-105 transition-transform inline-flex items-center gap-4 shadow-2xl shadow-accent/20 font-display uppercase tracking-widest">
                            Browse the Library <ArrowUpRight size={24} />
                        </Link>
                    </div>
                </section>
            </main>
