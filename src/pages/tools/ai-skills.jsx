...
import { 
  ...
  Target, Zap, Search, ListFilter, Database, Globe, Command, Compass
} from 'lucide-react';
...
const NeuralNetwork = () => {
    ...
    useFrame((state) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            pointsRef.current.geometry.attributes.position.array[i * 3 + 1] = y + Math.sin(time + x * 0.7) * 0.045;
        }
...
export default function AiSkillsPage() {
    const content = pageContent['ai-skills'];
    ...
    return (
        <div className="min-h-screen bg-background transition-colors duration-500 overflow-hidden relative text-foreground font-body">
            ...
            <main className="relative z-10 pt-32 pb-24">
                ...
                <section className="px-8 lg:px-20 py-20 max-w-7xl mx-auto">
                    <div className="max-w-4xl">
                        ...
                        >
                            ...
                            <span className="text-xs font-mono text-accent uppercase tracking-[0.3em] font-bold">Search · Explore · Deploy</span>
                        </motion.div>
                        ...
                        >
                            <Link to="/contact" className="px-10 py-5 bg-accent text-white font-bold rounded-2xl flex items-center gap-3 hover:scale-105 transition-all shadow-xl shadow-accent/20 text-lg uppercase tracking-widest font-display">
                                Access Directory <ArrowUpRight size={20} />
                            </Link>
                        </motion.div>
                    </div>
                </section>
                ...
                <section className="px-8 lg:px-20 py-24 border-y border-border/50 bg-card-bg/5 backdrop-blur-xl">
                    <div className="max-w-7xl mx-auto grid md:grid-cols-[1fr_450px] gap-20 items-center">
                        <div className="glass-card p-12 md:p-16 border-l-4 border-l-accent relative overflow-hidden group">
                            ...
                            <h2 className="text-3xl md:text-4xl font-display font-bold mb-8 italic">"Total <span className="text-accent underline decoration-accent/20 underline-offset-8">Visibility</span> Into Possible."</h2>
                            <div className="space-y-6 text-secondary text-lg leading-relaxed font-body">
                                <p>{content.whatItIs}</p>
                                <p>
                                    The Directory serves as the definitive reference for the OpenClaw ecosystem. Every skill is meticulously catalogued with schema definitions, integration requirements, and security compliance metrics, ensuring you have all the intel required for strategic deployment.
                                </p>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            {[
                                { icon: <Search />, title: "Filtered", desc: "Instant intent-based search." },
                                { icon: <ListFilter />, title: "Categorized", desc: "Logic-driven taxonomies." },
                                { icon: <Database />, title: "Indexed", desc: "Deep metadata architecture." },
                                { icon: <Globe />, title: "Universal", desc: "Platform-agnostic access." }
                            ].map((item, i) => (
                                <motion.div 
                                    key={i}
                ...
                {/* 3. The Discovery Protocol (Process) */}
                <section className="px-8 lg:px-20 py-32 max-w-7xl mx-auto">
                    <div className="text-center mb-24">
                        <h2 className="section-title mb-6 font-display uppercase tracking-widest">Find Your <span className="text-accent underline decoration-accent/10">Edge</span></h2>
                        <p className="text-secondary max-w-xl mx-auto text-lg font-body">Navigating the frontier of enterprise AI execution.</p>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-16 text-center relative z-10">
                        {content.results.map((metric, i) => (
                            <div key={i} className="flex flex-col items-center p-8 border-r last:border-0 border-white/10 group">
                                <Command className="w-12 h-12 mb-8 opacity-40 group-hover:opacity-100 transition-opacity" />
                ...
                {/* 5. Directory Intelligence */}
                <section className="px-8 lg:px-20 py-32 border-b border-border/50">
                    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-24">
                        <div className="space-y-16">
                            <div>
                                <h2 className="section-title !text-4xl text-left italic mb-8">System <span className="text-accent underline decoration-accent/20">Taxonomies</span></h2>
                                <p className="text-secondary text-lg mb-12 font-body max-w-xl">
                                    Our directory is updated daily, ensuring you have access to the absolute latest advancements in the OpenClaw skill ecosystem.
                                </p>
                            </div>
                        ...
                        <div className="glass-card !p-12 space-y-12 bg-card-bg/20 border-l border-border/50">
                            <div className="flex flex-col gap-10">
                                <h3 className="text-xs font-bold uppercase tracking-[0.5em] text-accent flex items-center gap-4">
                                    <Target size={20} /> Target Segments
                                </h3>
                            ...
                            <div className="flex flex-col gap-10">
                                <h3 className="text-xs font-bold uppercase tracking-[0.5em] text-accent flex items-center gap-4">
                                    <Zap size={20} /> Discovery Friction
                                </h3>
                ...
                <section className="px-8 lg:px-20 py-24 text-center">
                    <div className="glass-card max-w-4xl mx-auto p-16 md:p-24 border-t-8 border-t-accent shadow-2xl relative overflow-hidden group">
                        ...
                        <h2 className="text-5xl md:text-7xl font-display font-bold mb-10 tracking-tighter">Explore the <span className="text-accent italic">Possible</span></h2>
                        <p className="text-xl text-secondary mb-12 max-w-xl mx-auto leading-relaxed font-body">
                            The collective intelligence of OpenClaw is at your fingertips. Discover the skill that unlocks your next breakthrough.
                        </p>
                        <Link to="/contact" className="px-12 py-6 bg-accent text-white font-bold rounded-2xl text-xl hover:scale-105 transition-transform inline-flex items-center gap-4 shadow-2xl shadow-accent/20 font-display uppercase tracking-widest">
                            Explore the index <ArrowUpRight size={24} />
                        </Link>
                    </div>
                </section>
            </main>
